            ...
            <template v-slot:top>
              <v-toolbar flat color="white">
                <v-row>
                  ...
                  <v-col cols="2" sm="2">
                    <v-select
                      v-model="searchCondition.departId"
                      :items="departItems"
                  ...
                  <v-col cols="2" sm="2">
                    <v-select
                      v-model="searchCondition.positionId"
                      :items="positionItems"
                  ...
                  <v-col cols="1" sm="1">
                    <v-icon large color="blue darken-2" @click="getAllData()">search</v-icon>
                  </v-col>
                  ...
                  <v-divider class="mx-4" inset vertical></v-divider>
                  <v-col style="text-align:right" cols="2" sm="2">
...
export default class PositionMng extends Vue {
  ...
  async getAllData(): Promise<void> {
    this.items = [];
    await axios
      .get("http://localhost:9000/api/v1/staff/search-list", {
        params: {
          positionId: this.searchCondition.positionId,
          departId: this.searchCondition.departId
        }
      })
      .then(response => {
        if (response.data.data) {
  ...
  async deleteItem(item: SearchDataList) {
    ...
    if (warning1) {
      await axios
        .delete(`http://localhost:9000/api/v1/staff/${item.staffId}`)
        .then(response => {
          if (response.data.status === "SUCCESS") {
            this.getAllData();
          }
